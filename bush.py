...
class Bush(pygame.sprite.Sprite):
  ...
  def set_ripeness(self):
    self.image = self.original_image.copy()
    self.image = pygame.transform.scale(self.image, (int(self.rect.width*self.ripeness), int(self.rect.height*self.ripeness)))

  def eat(self, currDay):
    if(self.ripeness > .6):
      self.nextgrowth = currDay + 1
      self.ripeness = 0.1
      return True
